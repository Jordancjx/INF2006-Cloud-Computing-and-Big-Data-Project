...
def salary_employment_correlation(csv_path, year=None):
    """
    ...
    # Aggregate by school (to reduce noise from individual degrees)
    school_agg = year_df.groupby("school").agg({
    ...
    # Round values
    school_agg["employment_rate_overall"] = school_agg["employment_rate_overall"].round(1)
    school_agg["employment_rate_ft_perm"] = school_agg["employment_rate_ft_perm"].round(1)
    school_agg["gross_monthly_median"] = school_agg["gross_monthly_median"].round(0)
    school_agg["basic_monthly_median"] = school_agg["basic_monthly_median"].round(0)
    
    # Calculate correlation coefficient
    correlation_coefficient = calculate_correlation(
        school_agg["gross_monthly_median"].values,
        school_agg["employment_rate_overall"].values
    )
    ...
    for _, row in school_agg.iterrows():
        scatter_data.append({
            "degree": row["school"],  # Using school name as label
    ...
    avg_salary = school_agg["gross_monthly_median"].mean()
    avg_employment = school_agg["employment_rate_overall"].mean()
    max_salary_school = school_agg.loc[school_agg["gross_monthly_median"].idxmax()]
    min_salary_school = school_agg.loc[school_agg["gross_monthly_median"].idxmin()]
    ...
    return {
        ...
        "statistics": {
            ...
            "highest_salary_school": {
                "name": max_salary_school["school"],
                "salary": max_salary_school["gross_monthly_median"],
                "employment_rate": max_salary_school["employment_rate_overall"]
            },
            "lowest_salary_school": {
                "name": min_salary_school["school"],
                "salary": min_salary_school["gross_monthly_median"],
                "employment_rate": min_salary_school["employment_rate_overall"]
            },
